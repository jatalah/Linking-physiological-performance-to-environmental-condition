...
#Read the Excel file
data <- read_excel("data/processed/combined_median_prevalence_restored_9Sept26.xlsx") # this one wouldn't open
...
subset_cor_matrix[abs(subset_cor_matrix) < 0.50] <- NA
...
# Define full path
output_path <- "C:/Users/rebeccamc/OneDrive - Cawthron/General - Environmental Health Measures for Open Ocean Aquaculture/Objective 3/Data analysis/Obj3_analysis/Lab data/results/Brachiopods/correlation_groups_B.xlsx"
...
correlation_network_plot_B_lab<-ggraph(g_tidy, layout = "fr") +
  geom_edge_link(alpha = 0.5) +
  geom_node_point(aes(color = cluster), size = 5) +
  geom_node_text(aes(label = name), repel = TRUE, size = 3) +
  theme_void() +
  labs(title = "Brachiopods (lab experiment) - Highly Correlated Variable Groups (r > 0.9)")

ggsave(
  "results/Brachiopods/correlation_network_plot_B_lab.svg",
  plot = correlation_network_plot_B_lab,
  width = 10,
  height = 8,
  dpi = 300,
  bg = "white"
...
write_xlsx(unclustered_vars_df,
           "results/Brachiopods/unclustered_variables_B.xlsx")
...
# Save to Excel
write_xlsx(best_vars_df, "C:/Users/rebeccamc/OneDrive - Cawthron/General - Environmental Health Measures for Open Ocean Aquaculture/Objective 3/Data analysis/Obj3_analysis/Lab data/results/Brachiopods/best_vars_comparison_B.xlsx")
...
library(lares)
corr_var(numeric_data, c18_1n9c_oleic_acid_Gonad_median, top = 40) + labs(title = "Atrina – oleic acid (DG)")
